...
def generate_ide_branding(api_key, nama_produk, deskripsi_produk):
    ...
    4. ALASAN PEMILIHAN: Berikan analisa mendalam kenapa nama dan slogan ini cocok, bagaimana psikologi marketingnya, dan kenapa ini dijamin aman dari hak cipta.
    
    Gunakan format Markdown ini untuk setiap rekomendasi:
    
    🔥 **OPSI [Nomor]: [NAMA BRAND]**
    🗣️ **Slogan:** "[Slogan absurd dan catchy]"
    🧠 **Analisa & Alasan:** [Jelaskan secara detail dan asyik kenapa nama ini jenius, apa daya tariknya buat pasar sekarang, dan konfirmasi keamanan hak ciptanya]
...
if login_dengan_api():
    ...
    st.sidebar.markdown("---")
    st.sidebar.info("Portal ini mendesain nama brand yang unik, absurd ala jaman *now*, dan dijamin aman dari benturan hak cipta (HAKI).")

    # HALAMAN UTAMA
    st.title("👽 Portal Nama & Slogan Bisnis Absurd")
    st.write("Bosan dengan nama bisnis yang gitu-gitu aja? Masukkan produkmu di sini dan biarkan AI meracik nama brand + slogan tergila yang bakal bikin kompetitor ketar-ketir!")
    st.markdown("---")
    ...
    if st.button("🚀 Generate Nama Brand Sekarang!"):
        if not input_produk or not input_deskripsi:
            ...
            with st.spinner("Memindai tren bahasa Jaksel, Gen-Z, dan memverifikasi database Hak Cipta di kepala AI..."):
